function output= unfoldStruct2table(SC,varargin)

%Based on Unfold from:https://de.mathworks.com/matlabcentral/answers/94122-is-there-a-function-that-will-display-all-the-fields-of-a-nested-structure-in-matlab
...
%modified by Jan Eckhardt, FMI, Basel
    ...
    for h=1:hmax
        ...
        for i=1:NF
            ...
            if isstruct(SC(h).(F{i}))
                helper.unfold(SC(h).(F{i}),Namei,show);
            else
                if iscell(SC(h).(F{i}))
                    siz = size(SC(h).(F{i}));
                    NC = numel(SC(h).(F{i}));
