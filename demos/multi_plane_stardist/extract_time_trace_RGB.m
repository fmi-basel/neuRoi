...
myexp.planeNum = 3; % the plane number to open a trial, can also be modified in the GUI later
...
planeDir = sprintf('plane%02d',planeNum);
rgbSubdir = fullfile(rgbDir,planeDir);
if ~exist(rgbSubdir, 'dir')
    mkdir(rgbSubdir)
end
mapTypeList = {'anatomy','response','localCorrelation'};
mapIndList = [1, 2, 4];
mapDataList = uint8(zeros([trial.getMapSize(), 3]));
for k=1:length(mapTypeList)
    mapType = mapTypeList{k};
    mapInd = mapIndList(k);
    mapp = trial.getMapByInd(mapInd);
...
%% Step 05 (Maual ROI)
% If you draw ROIs only manually, then just double click the file to open a trial, draw ROIs by pressing 'f' on the keyboard.
% After done, save the ROIs by clicking File menu > Save ROis or pressing Ctrl+s on keyboard.
...
maskSubdir = fullfile(maskDir,planeDir)
...
fileIdxList = myexp.trialTable.fileIdx;
%% Step08 Extract time trace with template ROI in all trials
