...
font_size    = 11;
col          = 'k';
screen_size  =  thirdPartyTools.structdlg.get_screen_size('char');
...
if (isstruct(struct_def)) % Init
   ...
   h_fig = figure( ...
      ...
      'Units',               'char', ...
      'position',            fig_pos, ...
      'keypress',            'thirdPartyTools.structdlg.StructDlg(get(gcbf,''CurrentCharacter''));', ...
      'color',               get(0,'DefaultuicontrolBackgroundColor'),...
      'Visible',             visible,...
      'DeleteFcn',           'thirdPartyTools.structdlg.StructDlg;', ...
      'CloseRequestFcn',     'thirdPartyTools.structdlg.StructDlg(''cancel'');',...
   ...
   if (OK_vert_pos < 0)
      ...
      h_slider = uicontrol(h_fig, ...
         'style',         'slider', ...
         'callback',      'thirdPartyTools.structdlg.StructDlg(''slider_change'');', ...
         'Units',         'char', ...
   ...
   h_OK = uicontrol(h_fig, ...
      'style',         'pushbutton', ...
      'callback',      'thirdPartyTools.structdlg.StructDlg(''ok'');', ...
      'Units',         'char', ...
   ...
   h_reset = uicontrol(h_fig, ...
      'style',         'pushbutton', ...
      'callback',      'thirdPartyTools.structdlg.StructDlg(''reset'');', ...
      'Units',         'char', ...
   ...
   h_cancel = uicontrol(h_fig, ...
      'style',         'pushbutton', ...
      'callback',      'thirdPartyTools.structdlg.StructDlg(''cancel'');', ...
      'Units',         'char', ...
...
elseif (isstr(struct_def))
   ...
   if (~isempty(cmd))
      switch (cmd)
         ...
         case 'undo'
            undoCB(args{1},gcbf);
            thirdPartyTools.structdlg.StructDlg(args);
      ...
      if (isfield(ud.protected,fnames{i}))
         ...
         for hi = 1:length(cmenu)
            item = uimenu(cmenu(hi), ...
               ...
               'Callback',      ['thirdPartyTools.structdlg.StructDlg(''unlock(' fnames{i} ')'')'] );
         end
      end
   end
end
...
self_ref = thirdPartyTools.structdlg.struct_mfile_reference('','this',{str});
if (isempty(self_ref))
   return;
end
...
if (isempty(h))
   ...
   item1 = uimenu(cmenu, ...
      'Label',     item_label, ...
      'Callback',  ['thirdPartyTools.structdlg.StructDlg(''reset(' f ')'')'] );
   item2 = uimenu(cmenu, ...
      'Label',     'Undo', ...
      'Enable',    'off', ...
      'Separator', 'off', ...
      'Tag',       [f '_UNDO'], ...
      'Callback',  ['thirdPartyTools.structdlg.StructDlg(''undo(' f ')'')'] );
   ...
   h = uicontrol(h_fig, ...
      ...
      'Tag',        f, ...
      'UIContextMenu', cmenu, ...
      'Callback',   ['thirdPartyTools.structdlg.StructDlg({''' f '''});']);
...
if (isfield(ud.vals,f));
   [sub_vals,sub_units] = thirdPartyTools.structdlg.StructDlg(val,'',getfield(ud.vals,f),[],'off'); 
else
   [sub_vals,sub_units] = thirdPartyTools.structdlg.StructDlg(val,'',dflt_val,[],'off');
end
...
if (isempty(h))
   ...
   push_ud.cmd    = 'thirdPartyTools.structdlg.StructDlg';
      ...
      switch (push_ud.cmd)
         ...
         case 'StructDlg'
            if (isempty(ud.error))
               ...
               ret_struct = thirdPartyTools.structdlg.StructDlg(getfield(ud.orig_def,f),title,dflt_val,rec_pos,'on',v);
